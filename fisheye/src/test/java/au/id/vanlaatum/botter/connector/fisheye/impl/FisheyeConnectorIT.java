...
public class FisheyeConnectorIT {
  @Inject
  ...
  public Option[] configure () throws IOException, URISyntaxException {
    String version = mavenBundle ( "au.id.vanlaatum.botter", "fisheye" ).versionAsInProject ().getURL ().split ( "/" )[2];
    return options (
        cleanCaches (),
        junitBundles (),
        karafOptions (),
        bundleLink ( "au.id.vanlaatum.botter.core" ),
        bundle ( "file:" + PathUtils.getBaseDir () + "/target/fisheye-" + version + ".jar" )
    );
  }
  ...
  private Option karafOptions () throws URISyntaxException {
    ...
    return composite (
        ...
        features ( featuresPath + "features.xml", getRequiredFeatures ( featuresPath + "features.xml", "botter", "botter-fisheye" ) )
    );
  }
